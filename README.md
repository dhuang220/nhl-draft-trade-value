...
- **Player and pick values had to be put on the same timescale, after a real red flag.** Picks are valued as an expected *career* total (the pick-value curve), but an early version of the trade grader valued live/season-specific players as a single *season* total - so a late 1st-round pick's whole-career expectation looked almost as valuable as one season of Connor McDavid. Fixed by projecting a player's season rate forward over a realistic full career (the same games-played-by-draft-slot curve Draft Explorer uses), putting both sides of a trade on a comparable career-length scale. The one remaining inconsistency: the high-confidence 2016-17 precomputed table is still season-scale (it has no draft slot to project from without a fragile extra join) - documented rather than silently left as-is.
...
- **Draft Explorer coverage is 2000-2020 (Kaggle) plus 2023-2026 (live NHL API) - 2021-2022 are a known gap, not yet covered by either source.** The 2023+ classes come from the NHL's own draft-picks API (`api-web.nhle.com/v1/draft/picks/{year}/all`), which has no career-outcome stat of its own. Their "career Point Shares" column is a deliberate substitution: it's actually OUR composite value model's predicted career value (predicted per-game rate x games played, summed across every NHL regular season a player has appeared in so far), not the real Hockey-Reference stat - almost none of these very recent, mostly-not-yet-NHL players have a real one yet. Most rows are 0 because most of these picks haven't played an NHL game yet, which is the correct/expected outcome, not a data gap.
