...
def test_merge_draft_sources_keeps_2000_to_2020_kaggle_rows_and_all_recent_rows():
    kaggle = pd.DataFrame([_row(year=y) for y in [1999, 2000, 2019, 2020, 2021, 2022]])
    recent = pd.DataFrame([_row(year=y) for y in [2021, 2022, 2023]])

    out = merge_draft_sources(kaggle, recent)

    assert sorted(out["year"]) == [2000, 2019, 2020, 2021, 2022, 2023]


def test_merge_draft_sources_ignores_pre_2021_recent_rows():
    # A caller could in principle pass recent rows tagged with a year the Kaggle side
    # already covers - those should still be dropped rather than double-counted.
    kaggle = pd.DataFrame([_row(year=y) for y in [2019, 2020]])
    recent = pd.DataFrame([_row(year=y) for y in [2020, 2021]])

    out = merge_draft_sources(kaggle, recent)

    assert sorted(out["year"]) == [2019, 2020, 2021]
